...
func GetTLDRContent(n News, subject, year, month, date string) (News, error) {
	...
	url := "https://tldr.tech/"
	url += subject + "/" + fmt.Sprintf("%s-%s-%s", year, month, date)
	...
	c.OnHTML("div.mt-3:not(.text-center)", func(elem *colly.HTMLElement) {
		...
		if !success {
			slog.Warn("Could not cut link", "link", l) // TODO - is warn necessary?
		}
