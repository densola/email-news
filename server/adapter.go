...
// Misc Methods
func getDateNowString() string { // TODO - Given the method GetYMDNow(), is this one really needed?
	y, m, d := time.Now().Date()
...
// GetYMDNow returns the string versions of the current year, month, and day.
func GetYMDNow() (string, string, string) {
	y, m, d := time.Now().Date()
	...
	return fmt.Sprintf("%d", y), month, day
}
